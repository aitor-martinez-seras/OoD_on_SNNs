...
def load_DTD(batch_size, datasets_path: Path, test_only=False, image_shape=(3, 32, 32),
                      workers=4, *args, **kwargs):

    test_transform = load_test_presets(img_shape=image_shape)
    test_data = torchvision.datasets.DTD(
        root=datasets_path,
        split='test',
    ...
    if test_only is False:
        ...
        train_data = torchvision.datasets.DTD(
            root=datasets_path,
            split='train',
            download=True,
...
if __name__ == "__main__":
    dataset, train_loader, test_loader = load_DTD(

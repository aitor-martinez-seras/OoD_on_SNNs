...
class ConvSNN8(nn.Module):
    """
    No dropout, as ConvSNN11, but with same threshold for all layers
    """
    def __init__(self, input_size, hidden_neurons, output_neurons, alpha=100):
        # super(ConvNet, self).__init__()
        super().__init__()
        ...
        # LIF cells
        self.lif_conv1 = LIFCell(p=LIFParameters(v_th=torch.tensor(self.thr), alpha=alpha))
        self.lif_conv2 = LIFCell(p=LIFParameters(v_th=torch.tensor(self.thr), alpha=alpha))
        self.lif_conv3 = LIFCell(p=LIFParameters(v_th=torch.tensor(self.thr), alpha=alpha))

        self.lif_fc1 = LIFCell(p=LIFParameters(v_th=torch.tensor(self.thr), alpha=alpha))
        # self.lif_fc2 = LIFCell(p=LIFParameters(v_th=torch.tensor(0.1), alpha=alpha))
        self.out = LICell()
...
class ConvSNN9(nn.Module):
    """
    Three convs with padding and three poolings, 2 FC with dropout
    """
    def __init__(self, input_size, hidden_neurons, output_neurons, alpha=100):
        # super(ConvNet, self).__init__()
        super().__init__()
...
class ConvSNN11_no_dropout(nn.Module):
    def __init__(self, input_size, hidden_neurons, output_neurons, alpha=100):
        ...
        # Linear part
        self.fc1 = nn.Linear(4 * 4 * 128, hidden_neurons, bias=False)  # 2048
        self.fc_out = nn.Linear(hidden_neurons, output_neurons, bias=False)  # Out fc
...
class ConvSNN12(nn.Module):
    """
    As Conv11 but with one more linear layer
    """
    def __init__(self, input_size, hidden_neurons, output_neurons, alpha=100):
        super().__init__()
        ...
        # Linear part
        self.fc1 = nn.Linear(4 * 4 * 128, 256, bias=False)  # 2048 to 256
        self.fc2 = nn.Linear(256, hidden_neurons, bias=False)
        self.fc_out = nn.Linear(hidden_neurons, output_neurons, bias=False)  # Out fc
...
class ConvSNN14(nn.Module):
    """
    As Conv11 but without padding and without the first pooling
    """
    def __init__(self, input_size, hidden_neurons, output_neurons, alpha=100):
        super().__init__()
        ...
        # Linear part
        self.fc1 = nn.Linear(6 * 6 * 128, hidden_neurons, bias=False)  # 2048 to 256
        # self.fc2 = nn.Linear(256, hidden_neurons, bias=False)
        self.fc_out = nn.Linear(hidden_neurons, output_neurons, bias=False)  # Out fc
...
class ConvSNN15(nn.Module):
    ...
    def __init__(self, input_size, hidden_neurons, output_neurons, alpha=100):
        ...
        # Convolutions
        self.conv1 = nn.Conv2d(input_size[0], 16, 3, 1, bias=False)
        self.conv2 = nn.Conv2d(16, 32, 3, 1, bias=False)
        self.conv3 = nn.Conv2d(32, 64, 3, 1, bias=False)
        # self.pool1 = nn.AvgPool2d(kernel_size=2)
        self.pool2 = nn.AvgPool2d(kernel_size=2)
        # self.pool3 = nn.AvgPool2d(kernel_size=2)
        ...
        # Linear part
        self.fc1 = nn.Linear(12 * 12 * 64, 1152, bias=False)  # 9216 / 8 = 1152
        self.fc2 = nn.Linear(1152, hidden_neurons, bias=False)  # 1152 / 8 = 144
        self.fc_out = nn.Linear(hidden_neurons, output_neurons, bias=False)  # Out fc
        ...
        self.lif_fc1 = LIFCell(p=LIFParameters(v_th=torch.tensor(self.thr), alpha=alpha))
        self.lif_fc2 = LIFCell(p=LIFParameters(v_th=torch.tensor(self.thr), alpha=alpha))
        self.out = LICell()
...
class ConvSNN16(nn.Module):
    """
    4 convs, 2 poolings in the intermediate layers, no padding.
    """
    ...
    def __init__(self, input_size, hidden_neurons, output_neurons, alpha=100):
        ...
        self.conv2 = nn.Conv2d(16, 32, 3, 1, bias=False)
        self.conv3 = nn.Conv2d(32, 64, 3, 1, bias=False)
        self.conv4 = nn.Conv2d(64, 128, 3, 1, bias=False)
        # self.pool1 = nn.AvgPool2d(kernel_size=2)
        self.pool2 = nn.AvgPool2d(kernel_size=2)
        self.pool3 = nn.AvgPool2d(kernel_size=2)
        ...
        self.fc2 = nn.Linear(512, hidden_neurons, bias=False)  # 512 / 4 = 128
        self.fc_out = nn.Linear(hidden_neurons, output_neurons, bias=False)  # Out fc
        ...
        self.lif_conv2 = LIFCell(p=LIFParameters(v_th=torch.tensor(self.thr), alpha=alpha))
        self.lif_conv3 = LIFCell(p=LIFParameters(v_th=torch.tensor(self.thr), alpha=alpha))
        self.lif_conv4 = LIFCell(p=LIFParameters(v_th=torch.tensor(self.thr), alpha=alpha))
...
class ConvSNN17(nn.Module):
    """
    As Conv11 but three FC
    """
    def __init__(self, input_size, hidden_neurons, output_neurons, alpha=100):
        super().__init__()
    ...
    def forward(self, x, flag=None):
        ...
        # specify the initial states
        sconv1 = sconv2 = sconv3 = sfc1 = sfc2 = sfc3 = so = None
...
class ConvSNN18(nn.Module):
    """
    As Conv11 but less filters
    """
    def __init__(self, input_size, hidden_neurons, output_neurons, alpha=100):
        super().__init__()
...
class ConvSNN19_max_pooling(nn.Module):
    def __init__(self, input_size, hidden_neurons, output_neurons, alpha=100):
        # super(ConvNet, self).__init__()
        super().__init__()
        ...
        # LIF cells
        self.lif_conv1 = LIFCell(p=LIFParameters(v_th=torch.tensor(0.1), alpha=alpha))
        self.lif_conv2 = LIFCell(p=LIFParameters(v_th=torch.tensor(0.1), alpha=alpha))
        self.lif_conv3 = LIFCell(p=LIFParameters(v_th=torch.tensor(0.05), alpha=alpha))
...
class ConvSNN20(nn.Module):
    """
    As Conv11 but with one more CONV layer, no new avg pool and not avg pool in last conv
    """
    def __init__(self, input_size, hidden_neurons, output_neurons, alpha=100):
        super().__init__()
...
class ConvSNN21(nn.Module):
    """
    As Conv10 but dropout in the after the flatten
    """
    def __init__(self, input_size, hidden_neurons, output_neurons, alpha=100):
        # super(ConvNet, self).__init__()
        super().__init__()
    ...
    def forward(self, x, flag=None):
        ...
        # Dropout
        drop = nn.Dropout(p=0.4, inplace=True)
...
class ConvSNN22(nn.Module):
    """
    As Conv21 but less filters
    """
    def __init__(self, input_size, hidden_neurons, output_neurons, alpha=100):
        # super(ConvNet, self).__init__()
        super().__init__()
        ...
        # Convolutions
        self.conv1 = nn.Conv2d(input_size[0], 8, 3, 1, padding=1, bias=False)
        self.conv2 = nn.Conv2d(8, 16, 3, 1, padding=1, bias=False)
        self.conv3 = nn.Conv2d(16, 32, 3, 1, padding=1, bias=False)
        self.avgpool1 = nn.AvgPool2d(kernel_size=2)
        self.avgpool2 = nn.AvgPool2d(kernel_size=2)
        self.avgpool3 = nn.AvgPool2d(kernel_size=2)
...
class ConvSNN27(nn.Module):

    """
    2 convs no padding and avgpooling, one linear layer
    """
    def __init__(self, input_size, hidden_neurons, output_neurons, alpha=100):
        super().__init__()
        ...
        # Linear part
        self.features_out = self.ftmaps_h * self.ftmaps_v * 32
        # self.intermediate_linear_neurons = 512
        self.fc1 = nn.Linear(self.features_out, hidden_neurons, bias=False)
        # self.fc2 = nn.Linear(self.intermediate_linear_neurons, hidden_neurons, bias=False)
        self.fc_out = nn.Linear(hidden_neurons, output_neurons, bias=False)  # Out fc
        ...
        self.lif_fc1 = LIFCell(p=LIFParameters(v_th=torch.tensor(0.2), alpha=alpha))
        # self.lif_fc2 = LIFCell(p=LIFParameters(v_th=torch.tensor(0.1), alpha=alpha))
        self.out = LICell()
